                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        <TabsList className={`bg-white border border-slate-200 p-1 h-auto shadow-sm gap-1 self-start sm:self-auto overflow-x-auto max-w-full ${teamTabCount >= 7 ? 'w-full' : ''}`}>
                            ...
                            <TabsTrigger value="learnings" className="data-[state=active]:bg-emerald-50 data-[state=active]:text-emerald-700 flex items-center gap-1">
                            ...
                            {projectRejectionAlerts.length > 0 && (
                                <div className="space-y-3">
                                    {projectRejectionAlerts.map((project) => {
                                        ...
                                        const stepLabel = 'Step 1 — Team Setup';
                                        const note = (isSetup ? project.setupRejectionNote : '') ?? '';
                                        return (
                                            <div
                    ...
                    {/* TAB: LEARNINGS (FOR TEAM MEMBER PARTICIPANTS) */}
                    <TabsContent value="learnings" className="space-y-6">
                        {showReportStudio ? (
                            <LearningReportStudio
                                user={user}
                                onBack={() => setShowReportStudio(false)}
                            />
                        ) : (
                            <>
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-emerald-500/10 via-teal-500/10 to-blue-500/10 p-6 rounded-2xl border border-emerald-500/20">
                                    ...
                                    <div className="flex items-center gap-3 shrink-0 flex-wrap">
                                        <Button
                                            ...
                                            className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs flex items-center gap-2 rounded-xl text-xs font-semibold px-4 py-2 cursor-pointer transition-all"
                                        >
                                            <Edit3 className="w-4 h-4" />
                                            <span>Create Report</span>
                                        </Button>
                                        <div className="bg-white/80 backdrop-blur px-4 py-2 rounded-xl border border-slate-200 text-xs font-semibold text-slate-700 flex items-center gap-2">
                                ...
                                {attendedLearnings.length === 0 ? (
                                    <Card className="p-12 text-center border-dashed border-2 border-slate-200 bg-slate-50/50">
                                        ...
                                        <p className="text-sm text-slate-500 max-w-md mx-auto mb-4">
                                            Join published plans in the Learning Hub as a participant. Once session leaders complete attendance, your verified Learning Experiences will appear here!
                                        </p>
                                        <Link href="/learning">
